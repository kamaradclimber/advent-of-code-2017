...
cntj (57)' => 60
    ...
    context 'real input' do
      it 'has correct number of programs' do
        expect(subject.disks.size).to eq(input.split("\n").size)
      end

      it 'has correct number of towers' do
        expect(subject.disks.count { |_, d| d.children.any? }).to eq(input.split("\n").count { |l| l =~ /->/ })
      end

      context 'a random disk' do
        let(:uukembw) { subject.disks['uukembw'] }
        it 'a random disk is valid' do
          expect(uukembw.children.size).to eq(4)
          expect(uukembw.weight).to eq(1054)
        end
      end
